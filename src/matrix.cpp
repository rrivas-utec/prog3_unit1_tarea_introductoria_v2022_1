...
TYPE& utec::matrix::operator() (SIZE_TYPE row, SIZE_TYPE col) { return data[row*n_cols + col]; }
TYPE utec::matrix::operator() (SIZE_TYPE row, SIZE_TYPE col) const { return data[row*n_cols + col]; }
...
bool utec::matrix::operator==(const utec::matrix &other) const {
    ...
    auto result = true;
    for (size_t i = 0; i < n_rows && result; ++i)
